...
export const generatePDF = (
  ...
  doc.setFillColor(mode === 'patients' ? 14 : 79, mode === 'patients' ? 165 : 70, mode === 'patients' ? 233 : 229); 
  ...
  const totalProcessed = data.reduce((acc: number, curr: any) => acc + curr.totalProcessedValue, 0);
  ...
  if (mode === 'patients') {
    ...
    tableBody = (data as PatientSummary[]).map(item => [
      ...
      formatCurrency(item.totalProcessedValue), // Processado direto
